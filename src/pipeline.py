...
inference.py
------------
End-to-end inference pipeline for a match.

Given only the basic match info:
    home_team, away_team, city, country, date  (YYYY-MM-DD)

this script will:
    1. Geocode home/away team capitals + the stadium city         (geopy / Nominatim)
    2. Compute away_stadium_distance_km (haversine)
    3. Fetch historical weather for home/away/stadium locations    (Open-Meteo archive API)
    4. Fetch each team's last-2-match form + current ranking       (SofaScore, via Playwright)
    5. Build the same derived features used in training (cyclic encoding, stadium-diff
       temps/wind, ranking_diff, etc.)
    6. Scale with the saved RobustScaler and select the RFECV-chosen feature set
    7. Return win/draw/loss probabilities from the saved XGBoost model

If you already have a fully-engineered row (e.g. you built it yourself, or you're
re-running historical training data), pass `fetch_live=False` to `predict_proba`
and skip straight to scaling/prediction.
...
Dependencies beyond the original scaler/model bits:
    pip install requests geopy countryinfo playwright nest_asyncio
    python -m playwright install chromium

Rate limits respected:
    - Nominatim: ~1 request/sec
    - Open-Meteo archive: well under its 600/min limit
    - SofaScore: polite ~1 req/sec with a warmed-up browser session (same approach as
      the feature-engineering notebook, since it's an undocumented API scraped via a
      real browser context)

NOTE: I could not execute or test the network-calling code in this environment
(sandboxed, no access to sofascore.com / open-meteo.com / nominatim). It faithfully
mirrors the logic in feature-engineering.ipynb, but please smoke-test it against a
real match before relying on it.
"""
...
# If you have the team_ids.json produced in feature-engineering.ipynb, copy it into
# cache/ — it saves a live SofaScore search per team on every call.
TEAM_IDS_PATH = CACHE_DIR / "team_ids.json"

# Same manual overrides used in training, for teams whose "capital" needs help
# (CountryInfo doesn't know these are nations, or the pandas/geopy string doesn't match).
CAPITAL_OVERRIDES = {
    "England": "London",
    "Wales": "Cardiff",
...
# Raw columns that MUST come from the live-fetch steps (weather + sofascore + geocoding)
# before the derived-feature step can run.
_LIVE_FETCHED_COLS = {
...
# ══════════════════════════════════════════════════════════════════════════════
# 1. GEOCODING  (team capital / stadium city -> lat, lon, and away_stadium_distance_km)
# ══════════════════════════════════════════════════════════════════════════════
_geolocator = Nominatim(user_agent="nations-matches-prediction", timeout=10)


def _get_capital(team_name: str) -> str | None:
    if team_name in CAPITAL_OVERRIDES:
        return CAPITAL_OVERRIDES[team_name]
    try:
...
def _geocode_place(place: str, cache: dict) -> tuple | None:
    if place in cache:
...
def _add_location_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    cache = _load_cache(COORDS_CACHE_PATH)
    ...
    home_places = _places(df["home_team"])
    away_places = _places(df["away_team"])
    stadium_places = (df["city"].astype(str) + ", " + df["country"].astype(str)).tolist()

    def _geocode_list(places):
        coords = [_geocode_place(p, cache) if p else None for p in places]
    ...
    if "away_stadium_distance_km" not in df.columns:
        df["away_stadium_distance_km"] = _haversine(
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. WEATHER  (Open-Meteo archive API)
# ══════════════════════════════════════════════════════════════════════════════
def _fetch_weather(lat, lon, date: str, cache: dict) -> dict | None:
    ...
    for attempt in range(5):
        try:
            time.sleep(1.2)  # well under Open-Meteo's 600/min limit
            r = requests.get(
...
def _add_weather_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for prefix, lat_col, lon_col in [
        ...
        for lat, lon, date in zip(df[lat_col], df[lon_col], match_date):
            w = _fetch_weather(lat, lon, date, cache) if pd.notna(lat) and pd.notna(lon) else None
...
# ══════════════════════════════════════════════════════════════════════════════
# 3. SOFASCORE  (ranking + last-2-match form: avg starter rating, shots against, relative shots)
# ══════════════════════════════════════════════════════════════════════════════
...
async def _get_team_id(page, team_name: str, team_ids: dict) -> int | None:
    if team_name in team_ids:
        return team_ids[team_name]
    team_id = await _search_team_id(page, team_name)
    if team_id:
...
async def _last_2_form_stats(page, team_id: int, before_ts: int, cache: dict) -> dict:
    ...
    events = await _fetch_recent_finished_events(page, team_id, before_ts, max_events=3)
    ranking = None
    ...
    for event in events:
        ...
        if cache_key in cache:
            ...
            lineup = await _lineup_stats(page, event, team_id)
            stats = {
...
async def _fetch_sofascore_features_async(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    team_ids = _load_cache(TEAM_IDS_PATH)
    ratings_cache = _load_cache(RATINGS_CACHE_PATH)

    playwright, browser, page = await _make_browser_session()
    try:
        for idx, row in df.iterrows():
            ...
            for prefix, team_col in [("home", "home_team"), ("away", "away_team")]:
                team_id = await _get_team_id(page, str(row[team_col]).strip(), team_ids)
                if team_id is None:
                    continue
                stats = await _last_2_form_stats(page, team_id, before_ts, ratings_cache)
...
def _add_sofascore_features(df: pd.DataFrame) -> pd.DataFrame:
    return asyncio.get_event_loop().run_until_complete(_fetch_sofascore_features_async(df))


# ── Derived-feature engineering (mirrors feature-selection notebook) ──────────
